...
int main(int argc, char *argv[]){
	...
	if (argc > 1){
		...
		variables[names.back()].push_back(3.0);		
		variables[names.back()].push_back(9.0);		
	...
	for (auto&& [i,iterable] : iter::enumerate(iter::product(
		...
		system.q = 1;//2*pow(system.N,2)+4 + system.N; //std::max(q,int(pow(system.D,system.N))); // number of eigenvalues to consider
		system.sigma = sigma; // State shift parameter
		system.shift = 0; // State shift value
		// system.scale = 1; // State scale value
		if (sigma == "LA") {system.scale = -1;} else {system.scale = 1;}; // State scale value
		// system.s = 1; // number of unique eigenvalues to consider
		// system.q = 1;//2*pow(system.N,2)+4 + system.N; //std::max(q,int(pow(system.D,system.N))); // number of eigenvalues to consider
		// system.sigma = "0.7"; // State shift parameter
		// system.shift = 0;//100.0*(J*((2*d)*N/2/4) + h*N); // State shift value
		// system.scale = 1; // State scale value
